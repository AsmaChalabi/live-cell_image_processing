...
from skimage.exposure import match_histograms as hist_match
...
#############   TRACKING   #################
    ...
    def TrackCells( self,channel_class: str, bidirectional: bool = True, 
                   max_dist: float = np_.inf ) -> None:
...
def GetMultiSignals(self,cell_channel: None, nucl_channel:None, cyto_channel: None, features_channel: None,
    ...
    if features_channel != None:
        ...
        if cell_channel != None :
            ...
            features["cell"]["hist_argmax"]=[]
            features["cell"]["hist_entropy"]=[]
    ...
    for c_idx, root_cell in enumerate(self.RootCells()):
                                ...
                                if cmpt== 1 :
                                    ...
                                    if features_channel != None:
                                        ...
                                        if cell_channel!= None :
                                            ...
                                            hist,bins=np_.histogram(cell_frame[cell_.pixels], \
                                                                  density=True)
                                            
                                            cell_hist_argmax.append(np_.argmax(hist))
                                            cell_hist_entropy.append(-(hist*np_.log(np_.abs(hist))).sum())
        ...
        if features_channel != None:
            ...
            if cell_channel != None :
                ...
                features["cell"]["hist_argmax"].append(cell_hist_argmax)
                features["cell"]["hist_entropy"].append(cell_hist_entropy)
...
def GetUniSignal(self,channel: None, features_channel: None, class_: str):
    ...
    if features_channel != None and channel== features_channel :
        ...
        if class_=="cell":
            ...
            features[class_]["hist_argmax"]=[]
            features[class_]["hist_entropy"]=[]
    ...
    for c_idx, root_cell in enumerate(self.RootCells()):
                            ...
                            if max(node.pixels[0]) < frame.shape[0] and max(node.pixels[1]) < frame.shape[1]:
                                ...
                                if features_channel != None and channel== features_channel :
                                    ...
                                    if class_== "cell" :
                                        ...
                                        hist,bins=np_.histogram(frame[node.pixels], \
                                                density=True)
        ...
        if features_channel != None and channel== features_channel:
            ...
            if class_=="cell":
                ...
                features[class_]["hist_argmax"].append(cell_hist_argmax)
                features[class_]["hist_entropy"].append(cell_hist_entropy)
